...
import { getAllProperties, getCityCounts } from "@/utils/propertyApi";
...
export default function Properties() {
  ...
  useEffect(() => {
    Promise.all([getAllProperties(), getCityCounts()])
      .then(([props, cityCounts]) => {
        setProperties(props);
        const cityList = (cityCounts || []).slice(0, 5);
        setCities(cityList);
        if (cityList.length > 0) {
          setActiveCity(cityList[0].city || cityList[0]._id);
        }
      })
      .catch(console.error)
  ...
  useEffect(() => {
    ...
    setFiltered(
      properties.filter((p) =>
        ...
        p.country?.toLowerCase() === activeCity.toLowerCase() ||
        p.location?.toLowerCase().includes(activeCity.toLowerCase())
      )
    );
  }, [activeCity, properties]);

  const cityCount = (name) =>
    properties.filter(
      (p) =>
        p.city?.toLowerCase() === name.toLowerCase() ||
        p.country?.toLowerCase() === name.toLowerCase() ||
            ...
            {loading ? (
              ...
              <div className="widget-tabs style-2">
                <ul className="widget-menu-tab mb-48 overflow-x-auto">
                  {cities.map((c, i) => {
                    const name = c.city || c._id || c.name;
                    return (
                      <li
                        key={i}
                        className={`item-title hover-tooltip ${activeCity === name ? "active" : ""}`}
                        onClick={() => setActiveCity(name)}
                      >
                        {name}
                        <span className="tooltip">{cityCount(name)}</span>
                      </li>
                    );
                  })}
                </ul>
